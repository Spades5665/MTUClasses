...
def sentence3():
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    PacmanAlive0 = PropSymbolExpr('PacmanAlive', 0)
    PacmanAlive1 = PropSymbolExpr('PacmanAlive', 1)
    PacmanBorn0 = PropSymbolExpr('PacmanBorn', 0)
    PacmanKilled0 = PropSymbolExpr('PacmanKilled', 0)

    exp1 = PacmanAlive1 % ((PacmanAlive0 & ~PacmanKilled0) | (~PacmanAlive0 & PacmanBorn0))
    exp2 = ~(PacmanAlive0 & PacmanBorn0)
    exp3 = PacmanBorn0
...
def atMostOne(literals):
    ...
    for i in range(len(literals) - 1):
        for j in range(i + 1, len(literals)):
            truth.append(~literals[i] | ~literals[j])
    return conjoin(truth)
    "*** END YOUR CODE HERE ***"
...
def pacphysics_axioms(t, all_coords, non_outer_wall_coords):
    ...
    for x, y in non_outer_wall_coords:
        oneSquare.append(PropSymbolExpr(pacman_str, x, y, t))
    
    for action in ["North", "South", "East", "West"]:
        oneAction.append(PropSymbolExpr(action, t))

    pacphysics_sentences.append(exactlyOne(oneSquare))
...
def check_location_satisfiability(x1_y1, x0_y0, action0, action1, problem):
    ...
    pacmanIsAtXY = ~PropSymbolExpr(pacman_str, x1, y1, t)
    pacmanIsNotAtXY = PropSymbolExpr(pacman_str, x1, y1, t)

    model1 = findModel(conjoin(conjoin(KB), pacmanIsAtXY))
    model2 = findModel(conjoin(conjoin(KB), pacmanIsNotAtXY))
...
def positionLogicPlan(problem):
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            oneSquare.append(PropSymbolExpr(pacman_str, x, y, t))
        KB.append(exactlyOne(oneSquare))
